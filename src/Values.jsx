...
const STATUS_COLOR = { not_started: C.faint, in_progress: C.accentHot, done: C.accent };
...
export default function Values({ values = [], valueLinks = [], addValue, updateValue, removeValue, moveValue }) {
  ...
  return (
    <div>
              ...
              {editing ? (
                ...
                <>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: v.description ? 6 : 0 }}>
                    <Gem size={16} style={{ color: C.accent, flexShrink: 0 }} />
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 6, marginTop: 10, marginLeft: 24 }}>
                    {bare ? (
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 3, fontSize: 11.5, color: C.accent, background: C.accentSoft, padding: "2px 8px", borderRadius: 999 }}><Target size={10} /> нет целей</span>
                    ) : (
                      linkedGoals.map((g) => (
      ...
      {adding && (
        <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8, padding: 10, borderRadius: 16, background: C.surface, border: `1px solid ${C.accentEdge}` }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
            ...
            <button onClick={submitAdd} style={{ padding: "7px 14px", borderRadius: 10, fontSize: 13, fontWeight: 500, background: C.accent, color: C.onFill, border: "none", cursor: "pointer" }}>Добавить</button>
          </div>
